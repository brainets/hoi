...
def _oinfo_correct(inputs, index):
    hoi, acc = inputs
    msize = len(index)
    ...
    # get progress bar
    pbar = get_pbar(
        iterable=range(order.min()+1, order.max() + 1), leave=False
    )

    if order.min()==order.max()==2:
    ...
    for msize in pbar:
        pbar.set_description(desc="Oinfo correction (%i)" % msize, refresh=False)
